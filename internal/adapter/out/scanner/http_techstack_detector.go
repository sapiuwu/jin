...
// HTTPTechStackDetector is an out.TechStackDetector adapter that fetches
// the target page once and matches response headers, HTML markup, cookies,
// and script URLs against a built-in signature set.
...
// Detect implements out.TechStackDetector.
func (d *HTTPTechStackDetector) Detect(ctx context.Context, target string) (*domain.TechStackInfo, error) {
...
func runSignatures(d *scanData) []domain.Technology {
	...
	for _, sig := range signatures {
		...
		results = append(results, domain.Technology{
			Name:       sig.name,
